...
import { FormData, employeeData, Vendor } from '../../types/newDots';
...
const EmployeeInformationVendor: React.FC<PropsEmployeeInformation> = ({
    formData,
    setFormData,
    currentStep,
    setCurrentStep,
    ...
    useEffect(() => {
        const fetchEmployeeData = async () => {
            if (!formData.costCenter && !isTokenLoading) return;
            
            setLoading(true);
            try {
                const response = await axiosJWT.get(`${BpmsEndpoint}/employee?cost_center=${formData.costCenter}`);
                const employees = response.data || [];
                
                const filteredEmployees = employees.filter((emp:employeeData) => emp.partner.startsWith('13'));
        ...
        fetchEmployeeData();
    }, [formData.costCenter, axiosJWT, token]);

    // Filter employees based on search value
    const filteredEmployees = employeeData.filter(emp => 
        emp.partner.toLowerCase().includes(searchValue.toLowerCase()) ||
        emp.name_first.toLowerCase().includes(searchValue.toLowerCase())
    );
                                            ...
                                            <CommandList>
                                                ...
                                                <CommandGroup className="max-h-[250px] overflow-y-auto">
                                                    {(loading || isTokenLoading) ? (
                                                        ...
                                                        filteredEmployees.map((emp: employeeData) => (
                                                            <CommandItem
                                                                key={emp.partner}
...
export default EmployeeInformationVendor;