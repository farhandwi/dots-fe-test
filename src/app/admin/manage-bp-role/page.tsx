...
export default function ManageRoleMapping() {
  ...
  const fetchMappings = async (page: number, perPage: number) => {
    ...
    try {
      const response = await axiosJWT.get(`${ApiRole}/mapping-bp-user-types`, {
        params: {
          page,
          per_page: perPage,
          ...appliedSearchValues
  ...
  const handleAdd = () => {
    router.push('/admin/manage-bp-role/add');
  };

  const handleEdit = (mapping: RoleMapping) => {
    router.push(`/admin/manage-bp-role/edit/${mapping.BUKRS}/${mapping.bp}/${mapping.cost_center}/${mapping.seq_number}/${mapping.user_role}`);
  };

  const handleDelete = async (bukrs: string, bp: string, cost_center: string, seqNumber: string, userRole: string) => {
    Swal.fire({
      title: 'Are you sure?',
      text: 'Do you want to delete this role mapping? This action cannot be undone.',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        try {
          if(cost_center !== ''){
            const response = await axiosJWT.delete(`${ApiRole}/mapping-bp-user-types/${bukrs}/${bp}/${cost_center}/${seqNumber}/${userRole}`);
            Swal.fire({
              title: 'Deleted!',
              text: 'The role mapping has been deleted successfully.',
              icon: 'success',
              timer: 2000,
            ...
            const response = await axiosJWT.delete(`${ApiRole}/mapping-bp-user-types/${bukrs}/${bp}/{costCenter}/${seqNumber}/${userRole}`);
            Swal.fire({
              title: 'Deleted!',
              text: 'The role mapping has been deleted successfully.',
              icon: 'success',
          ...
          Swal.fire({
            title: 'Error!',
            text: 'Failed to delete the role mapping. Please try again.',
            icon: 'error',
  ...
  if (!user) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        ...
        <span className="ml-2">Loading role mappings...</span>
      </div>
    );
  }
    ...
    <Sidebar user={user}>
      <div className="p-8 bg-white min-h-screen">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">Manage Role Mappings</h1>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            ...
            <div>
              ...
              <Input
                ...
                value={searchValues.user_role}
                onChange={handleSearchChange('user_role')}
                onKeyDown={handleKeyPress}
                className="mt-1"
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={searchValues.bp}
                onChange={handleSearchChange('bp')}
                onKeyDown={handleKeyPress}
                className="mt-1"
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={searchValues.cost_center}
                onChange={handleSearchChange('cost_center')}
                onKeyDown={handleKeyPress}
                className="mt-1"
              />
            </div>
        ...
        <Table className='mb-10'>
          ...
          <TableBody>
            {isTokenLoading || isLoadingData ? (
                ...
                <TableCell colSpan={10} className="text-center py-10">
                  <div className="flex flex-col items-center justify-center text-gray-500">
                    ...
                    <p className="text-sm text-gray-400 mt-2">
                      No role mappings are available for your search criteria
                    </p>
              ...
              <TableRow key={`${mapping.BUKRS}-${mapping.user_role}-${mapping.seq_number}`}>
                <TableCell className="text-center">{mapping.BUKRS}</TableCell>
                <TableCell className="text-center">{mapping.user_role}</TableCell>
                <TableCell className="text-center">{mapping.bp}</TableCell>
                <TableCell className="text-center">{mapping.cost_center}</TableCell>
                <TableCell className="text-center">{mapping.seq_number}</TableCell>
                <TableCell className="text-center">{mapping.create_by}</TableCell>
                <TableCell className="text-center">{mapping.create_date}</TableCell>
                <TableCell className="text-center">{mapping.modified_by || '-'}</TableCell>
                <TableCell className="text-center">{mapping.modified_date || '-'}</TableCell>
                <TableCell>
                  <div className="flex space-x-2 justify-center">
                    ...
                    <Button 
                      variant="destructive" 
                      size="sm" 
                      onClick={() => handleDelete(mapping.BUKRS, mapping.bp, mapping.cost_center, mapping.seq_number, mapping.user_role)} 
