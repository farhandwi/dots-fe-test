...
export default function EditRoleCostCenter({ params }: { params: ParamType }) {
  const ApiBackend = process.env.NEXT_PUBLIC_DOTS_BE_END_POINT;
  const router = useRouter();
  const [bukrs, costCenter, assignedCostCenter, seqNumber, userRole] = params.params;
  ...
  const [mappingData, setMappingData] = useState<CostCenterMappingData | null>(null);
  ...
  const fetchMappingData = async () => {
    try {
      const response = await axiosJWT.get(
        `${ApiBackend}/mapping-cost-center-user-types/${bukrs}/${costCenter}/${assignedCostCenter}/${seqNumber}/${userRole}`
      );
  ...
  const handleCostCenterSelect = (costCenter: CostCenterData) => {
    setMappingData(prev => prev ? { ...prev, assigned_cost_center: costCenter.cost_center } : null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Role Cost Center mapping updated successfully');
                ...
                <div className="space-y-2">
                  ...
                  <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {mappingData.assigned_cost_center || "Select cost center..."}
